...
void CAN_RX_TASK(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (g_bRXFlag2) //Received done
        {
            ...
            if ((sCANMessage.ui32MsgID) == NUM_1)
            {
                token = NUM_1;
                ...
                CAN0_TX_node2();//try 3
#endif
                if (*sCANMessage.pui8MsgData == NUM_1) /* reverse mode cw */
                {

                    vTaskDelay(NUM_1000);
                    GPIOPinWrite(GPIO_PORTF_BASE, (GPIO_PIN_1), 0); /* set low on led */
                    token = NUM_0;
                    if (PB == NUM_1) /*normal mode ccw*/
                    {
                        PB = NUM_0;

                        data = NUM_0;/*transmitted data*/

                        CAN0_TX_node2(data);/*transmit to node 2*/
                    }
                    else
                    {
